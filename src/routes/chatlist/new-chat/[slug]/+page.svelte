<script lang="ts">
	...
	async function getChat(username: string, message: string): Promise<string> {
		// If chat exists return chatID
		const chatRef = doc(db, `users/${data.self.uid}/chats/${username}`);
	...
	async function createChat(username2: string, message: string): Promise<void> {
		const uid1 = data.self.uid;
		const uid2 = data.uid;
		try {
			const chatID = await runTransaction(db, async () => {
				const username1 = await getSelfUsername();
				const id = await createChatDoc([username1, username2]);
	...
	async function createChatDoc(members: string[]): string {
...
<div class="container">
	<ChatHeader on:close={handleClose} {username} showCallButtons={false} status="online" />
	{#if chatID?.length > 0}
